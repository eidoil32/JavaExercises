...
public class Repository {
    ...
    public Repository(Path repositoryPath, boolean isOld) throws IOException, RepositoryException {
        this.currentPath = repositoryPath;
        initialisePaths();
    ...
    public Map<String, List<BasicFile>> scanRepository() throws IOException, MyFileException, RepositoryException {
        ...
        Folder rootFolder = new Folder(currentPath, LangEN.USER_ADMINISTRATOR);
        ...
        if (blob.getType() == eFileTypes.FOLDER) {
            ...
            for (File f : fileList) {
                if (!f.getName().equals(Settings.MAGIT_FOLDER)) {
                    if (f.isDirectory()) {
                        Folder temp = new Folder(f.toPath(), LangEN.USER_ADMINISTRATOR);
                        temp.setRootFolder(temp_RootFolder);
                        scanRecursiveFolder(temp_RootFolder, temp);
                    } else {
                        Blob temp = new Blob(f.toPath(), LangEN.USER_ADMINISTRATOR);
                        temp.setRootFolder(temp_RootFolder);
                        scanRecursiveFolder(temp_RootFolder, temp);
                    }
                }
    ...
    public void updateRepository(Map<String, List<BasicFile>> files) throws IOException, MyFileException {
    ...
    private void putFile(Map<BasicFile, Blob> files, BasicFile file, Folder rootFolder) throws IOException {
        if (file.getType() == eFileTypes.FOLDER) {
