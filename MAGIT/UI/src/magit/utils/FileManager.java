...
public class FileManager {
    ...
    public static void zipFile(BasicFile file, Path pathToObject) throws MyFileException {
        ...
        if (file.getType() == eFileTypes.FOLDER) {
            ...
            File folderTXT = new File(pathToObject.toString() + "\\tempfolder.magit");
            try {
        ...
        try {
            fos = new FileOutputStream(pathToObject.toString() + String.format("\\%s", ((Blob) file).getSHA_ONE()));
        } catch (FileNotFoundException e) {
            throw new MyFileException(eErrorCodes.OPEN_FILE_FAILED, pathToObject.toString() + String.format("\\%s", ((Blob) file).getSHA_ONE()));
        }
